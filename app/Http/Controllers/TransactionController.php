...
class TransactionController extends Controller
{
    ...
    public function incoming(Request $request) {
        ...
        $date = DB::select("SELECT GetDateTimeLocal() AS date")[0]->date;
        $transaction_id = DB::select("SELECT getIdTransaction(?) AS transaction_id",[session('invoice')])[0]->transaction_id;
    ...
    public function checkout(Request $request, $invoice) {
        ...
        foreach($product_order as $PO) {
            $id_purchases_item = DB::select("SELECT getIdPurchasesItem(?) AS id_purchases_item", [$PO->name])[0]->id_purchases_item;

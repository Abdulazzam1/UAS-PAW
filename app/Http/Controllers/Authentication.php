...
class Authentication extends Controller
{
    ...
    public function handleLogin(Request $request) {
        ...
        $Auth = DB::select("SELECT * FROM user WHERE email = '".$request->email."' AND password = '".$request->password."' ");
    ...
    public function handleRegister(Request $request) {
        ...
        $idUserNew = DB::select("SELECT getIdUser(?) AS idUserNew", [$request->email])[0]->idUserNew;
